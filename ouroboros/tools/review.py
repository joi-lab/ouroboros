...
PROXYAPI_CHAT_URL = "https://api.proxyapi.ru/openrouter/v1/chat/completions"
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "models": {
                            ...
                            "items": {"type": "string"},
                            "description": (
                                "Model identifiers to query (via ProxyAPI) "
    ...
    async with semaphore:
        try:
            resp = await client.post(
                os.environ.get("PROXYAPI_CHAT_URL", PROXYAPI_CHAT_URL),
                headers={
...
async def _multi_model_review_async(content: str, prompt: str, models: list, ctx: ToolContext):
    ...
    api_key = os.environ.get("PROXYAPI_API_KEY", "") or os.environ.get("OPENROUTER_API_KEY", "")
    if not api_key:
        return {"error": "PROXYAPI_API_KEY not set"}
    ...
    try:
        ...
        # Finally check headers
            ...
            for key, value in headers_dict.items():
                if key.lower() in {"x-proxyapi-cost", "x-openrouter-cost"}:
                    cost = float(value)
                    break
